...
~\\[-1.5cm]
Dark Matter makes up \SI{25}{\%} of the energy content of the entire Universe~\cite{?}.
There is plenty of astronomical evidence for Dark Matter from the rotational curves of galaxies~\cite{?}, over the collision of the bullet cluster~\cite{?} to the \gls{cmb}~\cite{?}.
It is not known to this day as what kind of particle(s) Dark Matter is realized, as all the evidence is based on observation of gravitational effects.
One promising candidate is the \gls{wimp}~\cite{?}.
We can search for \glspl{wimp} and other Dark Matter candidates via three different detection principles: production, indirect detection, and direct detection.
Production is the creation of a Dark Matter particle via inelastic scattering of Standard Model particles and is typically conducted in collider Experiments such as ATLAS and CMS at the LHC~\cite{?}.
Indirect detection requires observing the annihilation of two Dark Matter particles into Standard Model particles and is searched for with ground and orbit based telescopes.
Direct detection requires the scattering of a Dark Matter particle off of a Standard Model particle.
For this channel the \gls{tpc} with noble liquid gas target material has become the most popular and successful detector type.
The reason is their high sensitivity to small energy depositions of a dew keV and the potential to instrument large target masses.
While the most stringent limit so far are set by the findings of the first generation of ton scale \gls{lxe} \gls{tpc} experiments \oneton, LUX, and PandaX-II, the second generation experiments \nton, LZ, and PandaX-4T are in commissioning.
More efforts are invested to back up the findings of the first generation or to set more stringent limits even after the second generation with the planned next-generation experiment DARWIN~\cite{letter of intent..}.
To increase the probability of interaction of Dark Matter with the detector, the target mass is increased gradually with the experiment generations.
Also, novel detector parts and processes are investigated to further increase the performance of next-generation noble liquid gas \glspl{tpc}.
Some of these promising new techniques are studied with the \gls{xebra} that can accommodate one small scale \gls{tpc} with $ \approx \SI{0.8}{\kilogram} $ \gls{lxe} target.
Sophisticated data analysis is the key to understanding the performance of detectors based on established and novel technologies alike.
...
The principles of a \gls{tpc}, signal production, signal detection, as well as \gls{daq}, and further processing via \gls{strax} are discussed in this chapter.
These are necessary for understanding the subsequent analysis about the calibration of the \gls{tpc}.
...
\gls{xebra} is the Freiburg based \darwin~R\&D platform for small scale \glspl{tpc}.  % TODO check whether this is the first use of gls{xebra}. if not drop the "The" in the beginning
...
This changes the secondary scintillation mechanism for which we need stronger electric fields that, among others, are achieved by thinner electrode-grid wires.
...
A detailed description of the mentioned processes follows.
...
The liquid level of Xenon is placed right in the middle between gate and anode with a distance of \SI{2.5}{\milli\m} to each of them.
...
The meshes produce the same far-field, whereas in the near-field the electrons are focused towards the center of a mesh hexagons and the field is not as homogeneous which is negligible regarding drift times.
...
Then the signal, next to distinguishable noise, is dominated by single \gls{pe} signals, whereas the occurrence of multiple \gls{pe} is significantly smaller.
...
Electronegative parts in the Xenon are bound to the heated-up Zirconium plate in the getter and are thus remove.
We pass Xenon through the getter in the filling process to clean it of residual water and Oxygen.
...
Furthermore, active cooling takes place \emph{inside} the \gls{tpc}, and thus has to be conducted without particle exchange.
...
In a processing sense events require peaks, peaks require records and records require raw records.
...
Data kinds are usually comprised of several, separately loadable parts e.g. \emph{peak~positions}, \emph{peak~classification}; see fig.~\ref{fig:data-structure}.
...
\begin{figure}
\centering
\includegraphics[width=\figw\textwidth]{Figures/th.jpeg}  % {Figures/waveform.png}
...
\paragraph{\emph{x} and \emph{y}} denote the interaction positions in the horizontal plane.
...
The S2's detection time is dominated by the drift time of the electrons, which, at $ v_\mathrm{drift} \approx \SI{1.8}{\milli\meter\per\micro\second} $ is in the order of a few \SIrange{1}{10}{\micro\second}~\cite{Aalbers18}.
The detection time of S2s is thus by orders of magnitude larger than the one of S1s, while it is also dependant on the $ z $ position.
