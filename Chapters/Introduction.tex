...
As what kind of particle(s) Dark Matter is realized is not know to this day, as all the evidence is based on observation of gravitational effects.
...
Indirect detection requires observing the annihilation of two Dark Matter particles into Standard Model particles, whereas direct detection looks at the scattering of a Dark Matter particle off of a Standard Model particle.
For the latter the \gls{tpc} with noble liquid gas target material has become the most popular and promising detector type.
The reason is their high sensitivity and potential to operate with low background.
While the most stringent limit so far are set by the findings of the first generation of large scale \gls{lxe} \gls{tpc} experiments \oneton, LUX, and PandaX-II, the second generation experiments \nton, LZ, and PandaX-4T are in commissioning.
...
To increase the likeliness of interaction of Dark Matter with the detector, the target mass is increased gradually with the experiment generations.
Also, novelties to detector parts and processes are investigated to further increase the performance of next-generation noble liquid gas \glspl{tpc}.
