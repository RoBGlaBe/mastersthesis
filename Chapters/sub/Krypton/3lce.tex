...
For a large number of initial photons, this corresponds to the fraction of all photons arriving at a photo cathode over the total number of photons.
...
To avoid ambigouities we define the \gls{de} as
...
where are vary for different \gls{pmt} models.
...
\emph{The reflectivity of \gls{ptfe}} surfaces, here mainly the \gls{ptfe} reflectors, is with arround \SI{95}{\%} high for \gls{vuv} light.
...
the reflectivity is given by \cite{}  % TODO cite demtröder 2
\begin{alignat}{2}
...
This step is later done using simulation data from the \gls{nest} software.  % TODO cite
...
Since we are working on a relative scale and due to our how we switch to the absolute it's justified to correct for the mean.  % TODO hier weiter
