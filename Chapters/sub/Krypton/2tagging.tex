...
\paragraph{The time ordering cut} removes events in case they are not in agreement with the ordering of the decay chain of Krypton.
...
\begin{figure}[H]
    ...
    An event that does not have a S1 or S2 peak gets assigned a drift time of zero, corresponding to $ z = 0 $.
    This explains the overshoot at this point.
    The second bump is at the gate position of $ z = \SI{-2.5}{mm}$ and where we have wall events.
...
\paragraph{The fiducial volume cut} rejects all events whose position have been reconstructed into a specific volume in the center of the drift volume.
...
\paragraph{The false main S1 paring cut,} similarly to the \emph{false smaller S1 pairing cut} restricts the \gls{s1a} peak in order to avoid misclassification.
...
We remove this population with a cut in area such that we will not bias real Krypton S1 population in other runs that we use this cut on.
...
\paragraph{A homogeneous spacial distribution} of Krypton inside the \gls{tpc} is expected.
With the tagging of Krypton via the discussed cuts we can test this hypothesis against the data to test our selection.
In the histogram of events over equal volume spaced r-bins, fig.~(TODO ref. r plot here) we observe counts inside the fiducial r-cut of $r_\mathrm{max} = \SI{23}{\milli\m}$ that is in agreement with homogeneity.
...
The overshoot around $ r = \SI{30}{\milli\m} $ are most likely wall events on average reconstructed towards the center as well as real krypton events.
...
As we can see in fig.~(TODO ref decay time hist) a fit of the $ t_\mathrm{diff} $-hist yields an decay time parameter of XX \SI{}{\nano\s}. % TODO write number and error
